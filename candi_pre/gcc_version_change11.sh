...
# start or not?
read -p "Have you successfully installed gcc-11? [y/N]" if_installed
if [[ "$if_installed" =~ [yY] ]]; then
    echo ""
else
    echo "Please install gcc-11 before you executing this script!"
    exit
fi
read -p "Do you want to change the version(through creating symbolic links) of GCC to 11? [y/N] " change_choice
...
# --- Part 1: Create symbolic links ---
    ...
    sudo find "$source_dir" -maxdepth 1 -type f -name '*-11' -print0 | while IFS= read -r -d '' file; do
        original_file="$file"
        link_name=$(basename "$original_file" | sed 's/-11$//')
