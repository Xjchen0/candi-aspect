...
parse_version_sets() {
    ...
    while IFS= read -r line; do
        if [[ $line =~ ^{ ]]; then
            ...
            VERSIONS["${block}_${key}"]=$value
        fi
    done < version-set
}

# Interactive version selection
select_aspect_version() {
    echo "Available ASPECT versions:"
    # Unified version formatting with enhanced pattern matching
    # Format version display with robust pattern matching
    format_version() {
        echo "$1" | awk '{print $2}' |
            sed -E 's/"//g; s/^(aspect[_-]?|v[_-]?)//; s/[_-]+/./g; s/\.+/./g; s/\.$//'
    }
    echo "1) "${VERSIONS[0_aspect]}
    echo "2) "${VERSIONS[1_aspect]}
    echo "3) "${VERSIONS[2_aspect]}
    
    while true; do
        read -p "Select ASPECT version (1-3): " choice
        case $choice in
            1|2|3)
                SELECTED_BLOCK=`expr $choice - 1`
                break
                ;;
            *)
...
set_version_vars() {
    ASPECT_VERSION=${VERSIONS[${SELECTED_BLOCK}_aspect]}
    DEAL_II_VERSION=${VERSIONS[${SELECTED_BLOCK}_dealii]}
    P4EST_VERSION=${VERSIONS[${SELECTED_BLOCK}_p4est]}
    TRILINOS_VERSION=${VERSIONS[${SELECTED_BLOCK}_trilinos]}
    TRILINOS_MAJOR_VERSION=${TRILINOS_VERSION:0:2}
    
    # Clean version strings
    DEAL_II_VERSION=$(echo $DEAL_II_VERSION | sed 's/"//g')
    P4EST_VERSION=$(echo $P4EST_VERSION | sed 's/"//g')
    TRILINOS_VERSION=$(echo $TRILINOS_VERSION | sed 's/"//g')
}

# Main version selection flow
parse_version_sets
select_aspect_version
set_version_vars
...
PRE="candi_pre"
if [[ $choice =~ 1 ]] ; then
    chmod +x ${PRE}/"gcc_version_change11.sh" && sh ${PRE}/"gcc_version_change11.sh"
    GCC_VERSION=11
else
    chmod +x ${PRE}/"gcc_version_change10.sh" && sh ${PRE}/"gcc_version_change10.sh"
    GCC_VERSION=10
fi
chmod +x ${PRE}/"candi_pre.sh" && sh ${PRE}/"candi_pre.sh"
PREFIX=${PREFIX}"-gcc"${GCC_VERSION}
...
read -p "Clean your past build?(y/N)" choice2
case $choice2 in
    y|Y)
...
unset choice2
################################################################################
# create local.cfg and write variables
cp local_example.cfg local.cfg
echo "# set the version of dealii p4est trilinos" >>./local.cfg
echo "DEAL_II_VERSION="${DEAL_II_VERSION} >>./local.cfg
cp ./deal.II-toolchain/packages/p4est.package.example ./p4est.txt
vi p4est_version.txt
echo "VERSION="${P4EST_VERSION} > ./p4est_version.txt
cat p4est_version.txt p4est.txt > p4est_package.txt
mv ./p4est_package.txt ./deal.II-toolchain/packages/p4est.package
echo "TRILINOS_MAJOR_VERSION="${TRILINOS_MAJOR_VERSION} >>./local.cfg
# Set download tool
...
# install ASPECT
cd ${ORIG_DIR}
read -p "Ready to install aspect-"${ASPECT_VERSION}"? [Y/n]" ready_or_not
case $ready_or_not in
    Y|y)
        mkdir ${HOME}/aspect
        git clone https://github.com/geodynamics/aspect.git -b ${ASPECT_VERSION} ${HOME}/aspect/"aspect-"${ASPECT_VERSION}
        cp "./candi_pre/gcc_version_change"${GCC_VERSION}".sh" ${HOME}/"aspect/aspect-"${ASPECT_VERSION}
        cd ${HOME}/"aspect/aspect-"${ASPECT_VERSION} && mkdir build && cd build && echo "DEAL_II_DIR=${HOME}/${PREFIX_PATH}/"deal.II-"${DEAL_II_VERSION}/" && cmake -DDEAL_II_DIR=${HOME}/${PREFIX_PATH}/"deal.II-"${DEAL_II_VERSION}/ ..
        make -j8
        file=${HOME}/aspect/"aspect-"${ASPECT_VERSION}/build/aspect
        if [ ! -f "$file" ]; then
            echo "error!"
        fi
        ;;
...
unset value
unset choice
unset SELECTED_BLOCK
unset ASPECT_VERSION
unset DEAL_II_VERSION
unset P4EST_VERSION
unset TRILINOS_VERSION
unset TRILINOS_MAJOR_VERSION
unset VERSION
